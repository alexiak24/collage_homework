...
var apiKey = "G7ndFzq9jpKMnty7VVSqdhHvlP5oeT8r";
var db = "osc";
var coll = "osc_collection";

function setup(){
  name_field = createInput("Alexia");
  people = createSelect();
  ...
  var download_button = createButton("DOWNLOAD")
  download_button.mousePressed(download);

  text_field = createInput(scene);
  var text_button = createButton("ADD TEXT")
text_button.mousePressed(addText);
...
function download() {
  ...
  for (var i = 0; i < allElements.length; i++ ) {
    ...
    if (el.src === null) continue;
    // $(el.elt)
    // image(el, el.x, el.y, el.width, el.height);

    var a = $("<a>")
      .attr("href", el.attribute('src'))
...
function addText() {
  ...
  numberOfElements++;
  var elementID = "OSC_TEXT_"+ numberOfElements;
  var dom_element = createP(text_field.value());
  // if (w == -1){  //just dropped
  ...
  dom_element.position(200, 200);
  dom_element.size(w, h);
  dom_element.rotation = 0;
  ...
  dom_element.mouseMoved( function(){
    if(this == selectedElement && dragging == true){
      this.position(mouseX-150,mouseY-150);
    }
  });
  ...
  $(dom_element.elt).dblclick(function( ) {
    dom_element.rotation = (dom_element.rotation+45)%360;
    dom_element.style("transform", "rotate("+dom_element.rotation+"deg)");
  });
